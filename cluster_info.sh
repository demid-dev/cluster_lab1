...
num_cores=$(pbsnodes -a | awk '/np =/ { if ($3 > 0) { count[$3]++ }} END { for (i in count) print count[i], i " cores" }' | sort -n | uniq)

# Count the number of nodes with the same amount of memory
num_memory=$(pbsnodes -a | awk '/physmem =/ { if ($3 > 0) { count[$3]++ }} END { for (i in count) print count[i], i/1024/1024 " GB" }' | sort -n | uniq)
